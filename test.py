...
#     def googleNewsEmailLinkFinder(self, text):
#         temp = text.split("\r\n\r")[2].split("http")[-1].split("\n")[:-1]#map(lambda stringa: stringa.strip(), )
#         link = "".join(temp)
#         link = "http"+link.replace("2E", ".")#.replace("=","")
#         return link
    ...
    def updateProcessedDb(self):
        ...
        toDo = set(self.articleDumps.keys()[:1000]).difference(self.inProcessedArticlesDB)
        ...
        self.addToDB(toDatabase, "openCalis.db")
        self.updateSets()
...
#         return entities
    ...
    def termExtractor(self, curPosUid, data):
        ...
        entities = self.openCalis(text)#self.returnEntities(text)
        if entities:
